...
import body.misc_utils as utils
import body.dist_chamfer as ext
...
class SMPLifyLoss(nn.Module):
    ...
    def forward(self, body_model_output, camera, gt_joints, joints_conf,
                body_model_faces, joint_weights, ground_y=None, cam_R=None,
                use_vposer=False, pose_embedding=None,
                scan_tensor=None, visualize=False,
                scene_v=None, scene_vn=None, scene_f=None, ftov=None,
                **kwargs):
        projected_joints = camera(body_model_output.joints)  # joints.shape: 1, 118, 3
        ...
        if self.sdf_penetration and self.sdf_penetration_weight > 0:
            ...
            body_sdf = F.grid_sample(self.sdf.reshape(1, 1, grid_dim, grid_dim, grid_dim),
        ...
        if ground_y is not None and cam_R is not None:
            device = body_model_output.body_pose.device
            vertices = body_model_output.vertices
            cam_R_inv = torch.from_numpy(np.linalg.inv(cam_R)).to(device)
            vertices = torch.matmul(vertices[0], cam_R_inv)
            body_min = vertices[:, 1].min()
            ground_loss = torch.abs(body_min - ground_y).sum() * 1e3
            total_loss += ground_loss
            
        if visualize:
            print('total:{:.2f}, joint_loss:{:0.2f},  s2m:{:0.2f}, m2s:{:0.2f}, penetration:{:0.2f}, contact:{:0.2f}, ground_plane: {:0.4f}'.
                  format(total_loss.item(), joint_loss.item() ,torch.tensor(s2m_dist).item(),
                         torch.tensor(m2s_dist).item() ,torch.tensor(sdf_penetration_loss).item(), torch.tensor(contact_loss).item(), torch.tensor(ground_loss).item()))
        return total_loss
